...
public:
  DemoNode() : Node("DemoNode"), port_("/dev/ttyACM1", 115200)
  {
    ...
    port_.addReceiveCallback(std::bind(
      &DemoNode::serialCallback, this, std::placeholders::_1, std::placeholders::_2
    ));

    timer_ = this->create_wall_timer(
      500ms, std::bind(&DemoNode::timerCallback, this)
    );
  }

private:
  void serialCallback(const std::vector<uint8_t>& buf, size_t bytes_received)
  {
    (void)bytes_received;
